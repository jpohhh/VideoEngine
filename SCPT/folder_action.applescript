	...
	try
		...
		repeat until i > the (number of items of addedItems)
			...
			if DoEncode = 1 then
				...
				-- Try first with the AppPath, then let the script find the program on its own.
				try
					if AppPath is not running then
						--we're not currently encoding, log it, give the encoding script a kick in the arse
						do shell script "time=$(date +%Y%m%d-%H%M%S); echo $time Breakfast NOT reporting running, so asking encoder to start... >> " & BreakfastLongLog
						
						-- Send it to Breakfast
						...
						do shell script "time=$(date +%Y%m%d-%H%M%S); echo $time Breakfast is reporting running, so not asking encoder to start... >> " & BreakfastLongLog
					end if
				on error
					if "Breakfast" is not running then
						--we're not currently encoding, log it, give the encoding script a kick in the arse
						do shell script "time=$(date +%Y%m%d-%H%M%S); echo $time Breakfast NOT reporting running, so asking encoder to start... >> " & BreakfastLongLog
						
						-- Send it to Breakfast
						tell application "Breakfast"
							run
							activate
						end tell
					else
