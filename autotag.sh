...
do
	...
	#generate tags and tag with AtomicParsley
		...
		atomicparsley64 "$1" --title "$cnam" --artist "$cart" --year $cday --longDesc "$desc" --desc "$desc" --stik value=$stik --TVShowName "$cart" --TVEpisode $tven --TVNetwork $tvnn  --TVSeasonNum $season --TVEpisodeNum $episode --artwork $HOME/Library/Application\ Support/Engine/coverart.jpg --overWrite
